...
[pdc, n_resize, filt, verbose, traj, topo, at_sel, feat, pca, system_name, pdb_chain_id, output_dir] = load_md_args(argparse.ArgumentParser())
...
if not os.path.exists("%sresults"%root_dir):
    os.makedirs("%sresults"%root_dir)
out_idx = 1
output_folder = "%sresults/%s/%s_%i/"
while os.path.exists(output_folder%(root_dir, pdb_chain_id, system_name, out_idx)):
    out_idx += 1
output_folder = output_folder%(root_dir, pdb_chain_id, system_name, out_idx)
